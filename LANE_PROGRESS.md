...
| S2 parity fixes | blocked (2026-09-15) — Vehicle Motion can now insert, copy, delete and replace vehicles and add or delete grid rows; the first wind or joint function can be created; the force-deflection plot shows 3I's second curve, wind segments are dropdowns, the vehicle Title is editable, and bad input shows 3I's message. All of that is checked and correct in the Windows screenshots. The one thing left is the test robot: it still can't open the wind segment dropdown on its own, so its run is 47 of 48 and the item can't be called done. |
