...
!C
     ...
     &         I, J, J1, J2, JJ, K, LT, MC, N, NFJ, NJ, NJJ, 
     &         NK, NLT, NOUT, NREM, NX
!C
...
!C                                                                       
!C    Input no. of segments to contact each plane.                     
!C    Input Card F.1.a                                                  
!C                                                                      
      N = NPL / I_18
      NREM = MOD ( NPL, I_18 )
      IF ( LIN_FLAG )  THEN
       IF ( N .NE. I_0 )  THEN 
        ...
        END IF
       ELSE
        READ ( LULIN, * ) ( MNPL(J), J=1,NPL )                
       END IF
      ELSE
       READ  ( LUAIN, 100 )  ( MNPL(J), J=1,NPL )                         
  100  FORMAT ( 18I4 )                                                 
       IF ( AIN_CONVERT )  THEN
!C
!C      Determine the number of F.1.a cards needed and the
...
!C
  ...
  120     FORMAT ( A2 )
        ...
        END IF
!C
!C      Write out the F.1.b cards with a variable number of elements.
!C
        IF ( N .NE. I_0 )  THEN
         J1 = I_1
         J2 = I_18
         DO I=1,N
          WRITE ( LULIN, 125 ) ( MNPL(J), J=J1,J2 ), 'Card F.1.a'                
  125     FORMAT ( 1X, 18( I4, 1X ), 1X, A )                                                  
         ...
         IF ( NREM .NE. I_0 )  THEN
          IF ( NREM .LT. I_10 )  THEN
           WRITE ( LULIN, A_FRMT_26 ) ( MNPL(J), J=J1,J2 ), 
     &                                'Card F.1.a'
          ELSE
           WRITE ( LULIN, A_FRMT_27 ) ( MNPL(J), J=J1,J2 ), 
     &                                'Card F.1.a'
          END IF
         END IF
        ELSE
         IF ( NREM .LT. I_10 )  THEN
          WRITE ( LULIN, A_FRMT_26 ) ( MNPL(J), J=1,NPL ), 
     &                               'Card F.1.a'
         ELSE
          WRITE ( LULIN, A_FRMT_27 ) ( MNPL(J), J=1,NPL ), 
     &                               'Card F.1.a'
         END IF
        END IF
       END IF
      ...
      NJJ = NPL                                                       
!C                                                                      
!C    Start of loop to read contacts for planes. 
!C                                                                       
      MC = I_0                                                     
      BEGIN_LOOP = TRUE
      DO  J=1,NJJ                                                      
       ...
       IF ( NK .LE. I_0 )  CYCLE                                    
!C
!C     Write header to the standard output file for the plane-segment
!C      contacts.
!C
       IF ( BEGIN_LOOP ) THEN  
        WRITE ( LUAOU, 130 )                      
  130   FORMAT ( '0', 119X, 'Cards F.1' )                             
        WRITE ( LUAOU, 135 )                                
  135   FORMAT ( '0', 3X, '  Plane ', 8X, 'Segment', 
     &            2X, 'Force Deflection',
