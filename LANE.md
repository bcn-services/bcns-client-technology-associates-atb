...
- task: S2 parity fixes — close the S2 gaps against ATB 3I that `LANE_PROGRESS.md` "Next" lists (detail in
  ...
  guardrails:
    ...
    - Existing passing tests remain passing
  caution: true
  status: blocked (d177468, 2 sessions) — everything but the robot's wind SegID dropdown step is verified: delta QA over 3618cf2..d177468 PASS (1276 tests; TrimRows, the token-13 SegID post-step and two more guards each mutation-checked in isolation and restored byte-identical), delta review 0 Critical / 3 Important / 4 Minor with the earlier Importants confirmed fixed, and the two-series FDF, Copy Vehicle and row-cancelled screenshots are correct. Windows runs 35035065135 and 35036307936 are 47/48: S2ParityGridsAndFunctions still times out at "SegID list dropped" — F4, F2 + UIA `ComboBox.Expand()`, and a shown-List fallback all fail, so next try Alt+Down on the focused editing control, a mouse click on the drop-down button rect, or asserting the popup as a desktop-level "ComboLBox" window. Open review Important: the assertion pins the list at >1 item, not at the deck's segment count
